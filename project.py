...
def category_admin():
    if 'username' not in login_session:
        ...
        if (isAdmin(login_session) is True):
            categories = session.query(Category).all()
            user_logged = is_user_login()
            return render_template(
...
def admin_auth(user_id):
    ...
    if auth_user.id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized\
                to update this user. Please contact system administrator.');}\
...
def newCategory():
    if 'username' not in login_session:
        return redirect('/login')
    if (isAdmin(login_session) isFalse):
        return redirect(url_for('category_admin'))
...
def editCategory(categoryID):
    if 'username' not in login_session:
        return redirect('/login')
    if (isAdmin(login_session) is False):
        return redirect(url_for('category_admin'))
...
def viewItem(categoryID, item_id):
    ...
    if ('username' in login_session) and (
        ...
        return render_template(
            'viewitem.html', category=category, categoryID=categoryID,
            item=editedItem, user_logged=user_logged)
    else:
        return render_template(
            'publicviewitem.html', category=category, categoryID=categoryID,
            item=editedItem, user_logged=user_logged)
...
def newItem(categoryID):
    ...
    if (request.method == 'POST'):
        ...
        user_logged = is_user_login()
        return render_template(
                                'newitem.html', categoryID=categoryID,
                                categories=categories, user_logged=user_logged)
...
def editItem(categoryID, item_id):
    ...
    if request.method == 'POST':
        ...
        user_logged = is_user_login()
        return render_template(
            'edititem.html', categories=categories, categoryID=categoryID,
            item=editedItem, user_logged=user_logged)
...
def deleteItem(categoryID, item_id):
    ...
    if request.method == 'POST':
        ...
        user_logged = is_user_login()
        return render_template(
            'deleteitem.html', categoryID=categoryID, item=deletedItem,
            user_logged=user_logged)
